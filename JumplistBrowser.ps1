...
'@
			...
			# StringData:
			#		HasName
			$name = if ($LinkFlags.contains('HasName')) { $ShotcutInfo.StringData.NameString } else { $null }
			#		HasRelativePath
			...
			if ($LinkFlags.contains('HasLinkTargetIDList'))
			{
				...
				if ($len -gt 0)
				{
					...
					{
						if (![String]::IsNullOrEmpty([environment]::getfolderpath($known)) -and $ShotcutInfo.LinkTargetIDList.Path.Contains([environment]::getfolderpath($known)))
						{
							$linkpath = $ShotcutInfo.LinkTargetIDList.Path.Replace("$([environment]::getfolderpath($known))", "[$($known)]")
							break
						}
						else { $linkpath = $ShotcutInfo.LinkTargetIDList.Path.ToString() }
					}
					$displayname = $ShotcutInfo.LinkTargetIDList.ItemIDList.displayname[$dcount - 1]
				}
				else
				{
					$linkpath = $ShotcutInfo.LinkTargetIDList.Path.ToString()
					$displayname = $ShotcutInfo.LinkTargetIDList.ItemIDList.displayname[$dcount - 1]
				}
			}
		...
		# HasIconLocation
			...
			} else { $iconTargetAnsi = $iconTargetUnicode = $null}
			...
			else { $TargetAnsi = $TargetUnicode = $null }
		...
		$linktargets = [PSCustomObject]@{
				...
				'NetName'		   = $NetName
				'NetName Unicode'  = $NetNameUnicode
				...
				# HasExpIcon
				'iconTargetAnsi'   = $iconTargetAnsi
				'iconTargetUnicode'= $iconTargetUnicode
				# HasArguments
				'Command Args'	   = $CommandLineArguments
				
				# Byte Array
				'LNKByteArray' = $ByteArray
		}
	...
	function get-files
	{
		...
		{
			$Status.Text = "There are LNK/*-ms Files found in $($Folder)"
				...
				foreach ($offset in $offsets)
				{
					...
					if (!!$LNKData)
					{
						...
						if ($LNKData.Link_Flags.contains('HasLinkInfo') )
						{
							...
							# VolumeID
							if ($LNKData.LinkInfoFlags.Contains('VolumeID'))
							{
							...
							# LocalBasePath
							if ($LNKData.LinkInfoFlags.Contains('LocalBasePath'))
							{
							...
							# CommonNetworkRelativeLink
							if ($LNKData.LinkInfoFlags.Contains('CommonNetworkRelativeLink'))
							{
								...
								{
									$CommonNetworkflags = @($LNKData.CommonNetworkRelativeLinkFlags.ToString().split(',', [System.StringSplitOptions]::RemoveEmptyEntries))
								...
								if ($LNKData.CommonNetworkRelativeLinkFlags.ToString().Contains('ValidDevice'))
								{
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCC/tT4tuytM6Ual
# 52Wsy86m+BEwlzCI27v7W9mxg9Xu4KCCKI0wggQyMIIDGqADAgECAgEBMA0GCSqG
...
# AQQBgjcCAQQwLwYJKoZIhvcNAQkEMSIEINKJhvuQ66ZO2Yka+sTyr8mq8jBV9zdD
# K24zOMp5SlafMA0GCSqGSIb3DQEBAQUABIICAG7x90mPOfQYgguZs6X3v7VSOAI+
# f9c7CQr+LwL9q6BvMlrElCJqQuDD5KNC/QmruzpqXb6BF5y/KYbr6a5zzGUyX4OY
# dJdmiNJS7MBaXeJP6gzOwAm91NOEJhw9eHcWYcn40kfzbo82VABwVG7bGH5mcWCe
# CDqC+28z7HFiAzdHzudVc2xu0WdZ8g5luEq5xnO+GrEgy0PF6fw29CjQKwedaGs8
# 1n6fSD7loz0zJNIg6yKBR6L6zWMT3NHqg8oBUVQuxoOMAo+/uCoiTS8zD73ue1KQ
# aBOYW/8H/mS1HmmriQRviPxOxKBnqU9tbNDjYH9BxjJJZeqkuekLqUZ81dajGBxz
# lYZzXW8o8zyexSrhTG+hiocJqHFM1MxWhUosX3BNdmfvqilPTRWw0i8dB9VzWbUZ
# +sbzmPNcjDoTZ9b5kj3pfrIAWdZQ+jLN0IU6O4doe/DioqGZZFU4pqBg4eDINHH9
# HovzeZY3hnPqJ2oe3jpXbvPetBrJRNE+3TD7Q9ukQSg+wyLh7N7JOcLMqU0PxnNd
# 3yU/ncl9QRbdf53fg8c94pw3TptSvUP508hUiGJbq50xjlsV7iPy4whqlSarUaOW
# j7DVAsxGvQPTak4zUF+wAOHweb40cx4aC4N2xefXQSiiUpsYnTJXvUOj1WriBZty
# hg8b5kYMrEGn46haoYIDbDCCA2gGCSqGSIb3DQEJBjGCA1kwggNVAgEBMG8wWzEL
...
# hvcNAQcBMBwGCSqGSIb3DQEJBTEPFw0yMjEyMDQyMDAwMDRaMCsGCSqGSIb3DQEJ
# NDEeMBwwCwYJYIZIAWUDBAIBoQ0GCSqGSIb3DQEBCwUAMC8GCSqGSIb3DQEJBDEi
# BCAOyO6QymHYdl8cs6R6F5t12aXMt9OEcb320Sr8qYALtDCBpAYLKoZIhvcNAQkQ
...
# p6EZTQwcV7gpMA0GCSqGSIb3DQEBCwUABIIBgFFfP6F8Uz3mqGFvYMzphYnU8Wzr
# f8e37ZwNR6fh1FZqa3AbhI+vj2PnByOv0ZX1BW6DbESLDaLOCLqovn+GqJ7iMu+A
# 5yqzJWYSQH00ItwDpbBNFSVQSJZyRpTBA2Ck9z5QtPleyK4wrAGpLTJC1u5j81c5
# t13irvMWFPZnCGyA9NWEXIobxNXeakfsZAUzqKTsV6URtddE6yQldUHzi64Q+dn9
# WYyFRixGQF82bAFVRri7H3iwCFNxG4zDbSpPYMc17gXNIHzNwOvtkOaOPE0SGDpp
# 5rxfCV55t5pMERSPfoBrurPTQAMa8WaiKWmxrdYdAJgl6MBOh6a+4pVNoDX6Ino0
# zk4gMlsZ64vkNXNvm95k1B8IzVFr0mvUsrpiFcL+j2E/+SQs/HCzfWYj9HiESwg1
# vg8pNg5YjNOy0o5wy/yYfZ+eDhp76GYSBOTjPGyxAy/ByXJ8gSLycZQgmnne+dnd
# IfzxmGmpEFcmEQW1RtlO1BMTK7D4QMqkyyxmgg==
