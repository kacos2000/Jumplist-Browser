				...
				'00'{
					if ([System.BitConverter]::ToString($ByteArray[2]) -eq '2F')
					{
						...
						$ItemIdListProperties = [PSCustomObject]@{
							...
							'GUID'	     = if (!!$CLSID) { Get-FolderDescription -CLSIDstring $CLSID }else{ $null }
							'CLSID'	     = if (!!$CLSID0) { Get-CLSID -CLSIDstring $CLSID0 }else{ $null }
						}
					} # End of 0x2F
				...
				for ($t = 0; $t -lt $pc.PropertyStorage.Count; $t++)
				{
					...
					if ($pc.PropertyStorage[$t].PropertyStorage.Count -ge 1)
					{
						for ($p = 0; $p -lt $pc.PropertyStorage[$t].PropertyStorage.Count; $p++)
						{
							...
							elseif ($ttype.Contains('VT_I2, VT_VERSIONED_STREAM') -or ($ttype -eq 'VT_CF' -and $tname -notin $notunicode) -and $null -ne $typed.Value) #FMTID/uint32/Unicode
							{
								try
								{
							...
							$sfgoflags = if ($Description -eq 'SFGAOFlags [28636AA6-953D-11D2-B5D6-00C04FD918D0\25]')
											{ Get-SFGAOflags -Flags ($tpvalue/1) -Single $true } else { $null }
		...
		if ($dataoffsets.Count -ge 1) 
		{
			...
			foreach ($offset in $offsets)
			{
				...
				if (!!$LNKData)
				{
					...
					$LNKNode = $Root2.Nodes.Add("LNK #$($o)", "[$($offset.Start.ToString('D5'))] $($fname) LNK #$($o.ToString('D3')) - $($ShowName)")
...
target file size."
	...
	function Get-RecentDocs
	{
		...
		if ($registrykey.ValueCount -gt 0 -and $keyname -in ('RecentDocs'))
		{
			...
			$RecentNode.ImageIndex = 14
			$RecentNode.SelectedImageIndex = 15
				...
				if ($null -ne $keyValue)
				{
					try
					{
						...
						elseif ($keyname -in ('Streams'))
						{
							...
							$itemIDlist = if ($itemID -is [ShellLink.Structures.ItemID]) { Get-LinkTargetIdList -ItemIdListItem $itemID }else { $null }
						}
						else
						{
					...
					$Valuenode.SelectedImageIndex = 3
				}
			}
		}
	...
	function Get-NavPane
	{
		...
		foreach ($key in $keylist)
		{
			...
			$Root.Nodes["LastWriteTime"].ForeColor = 'Cyan'
			# Open Registry
			$Registry = [Microsoft.Win32.RegistryKey]::OpenBaseKey([Microsoft.Win32.RegistryHive]::CurrentUser, [Microsoft.Win32.RegistryView]::Default)
			$registrykey = $Registry.OpenSubKey($key)
			# $TreeSearch.Visible = $true
	...
	if (!!$treeview2)
		{
			...
			$LinkName = "$($treeview2.Nodes[0].Text.Replace($InvalidChars, '_')).json"
	...
	$treeview2_AfterSelect=[System.Windows.Forms.TreeViewEventHandler]{
		...
		# SaveLNK to JSON
		if ($treeview2.Nodes[0].Text.EndsWith('.lnk') -or $treeview2.Nodes[0].Text.EndsWith('-ms') -or $treeview2.Nodes[0].Text -eq 'Local Registry' -or $treeview2.Nodes[0].Text.EndsWith('NavPane') )
		{
...

...

...

...

...

...

...

...

...

...
AYABAAEBAYACAAGAAgABwAEAAQEBwAEAAQEBgAIAAYABAAEDAcABAAEBAcABAAEBAYABAAEBAYAB
AAEHAcABAAEDAcABAAEBAYABAAEBAYABAAEHAcABAAEHAcABAAEBAcABDwH/AYABDwH/AcABAAEH
AcABAAEBBv8BwAEAAQ8BwAEAAQEG/wHAAQABDwHAAQABAwb/AfABAAE/AfABAAEHDP8L'))
	#endregion
	...
	$imagelist1.Images.SetKeyName(14,'list-info.ico')
	$imagelist1.Images.SetKeyName(15,'list-ok.ico')
	#
	# imagelist2
	#
	#region Binary Data
...

...

...

...

...

...

...
'))
	#endregion
	...
	$imagelist2.Images.SetKeyName(14,'list-info.ico')
	$imagelist2.Images.SetKeyName(15,'list-ok.ico')
	#
	# toolstripseparator1
	#
...
A4LphFA6IJhOCKUDgumEUDogmE4IpQOC6YRQOgCFNm55Bi4AW/SimGtrwKkAAAAASUVORK5CYIIL'))
	...
	$GetNavPane.Size = New-Object System.Drawing.Size(267, 30)
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCBrTVs1O3KV3v/4
# qNBfE1b3BvV+RSY/ZobiiifaIzzTf6CCKI0wggQyMIIDGqADAgECAgEBMA0GCSqG
...
# AQQBgjcCAQQwLwYJKoZIhvcNAQkEMSIEINhBqijO0YGsTqJ7cfGD2YITLCfNRKxA
# JqvChs+a+czBMA0GCSqGSIb3DQEBAQUABIICABfDTFg9HI7yyLwQ0G8Hp4MmZS5h
# 32sNOqwgx0nIr21k0c/yzkcR3lbGSo4mq9/BGjl5SKYCJL8le0Sfhe/T7+x3NVqH
# DTxtK7c+5FfLvRH4YY9WPosXXcOzPdxGv2lmiV+RufYBIeJJeS7/RDnjws1oID9K
# zgb+NL8JSBcUH3cd5u8SuLqwWTtSA/04xogSw83QwklF+WA/5T4oVts+zUuuCU2g
# Zv71KjKn59YlY7KgW1Ht3RRyfxX4fLEyUQXQVJdmELHDIH7r9lUiWVGAZpQwKeJ7
# qqlg1nyLkVixpKwqmer5ukrb6xSCcMSHrJdZDOJeAUZdR8DHPraksKir54GTEASE
# vDYoADpInOI93Nebp6cynnWnE8A0+2xlaFUF8Sp4HLOSBmZRqwDQbraCLHGGn56q
# JhItFLHRUxzsY8qk/8l36tQu9HwDgPC+Eh56ZEk7Cums0W/NUKnc8WupZXe7OnMo
# zNMq9KwpcCuxxB8X0S4wvsZJU2CuDpXUcPWk14WpZPscMes/jGHatfqL4WbMniM1
# i1aaI5iHxcGteebz3RnLnSnx5RfvaSg3eGyC1fQs2janjn3Rh3hLyf5Z1hPjzoP+
# 95XvJCC9cJOfDvRXgqrKWAnCpVcN6DJNYD9m/DpzjqeXGCip67Tfcui1F+jVJR1z
# nm9CkVHbrn9KDGI4oYIDbDCCA2gGCSqGSIb3DQEJBjGCA1kwggNVAgEBMG8wWzEL
...
# hvcNAQcBMBwGCSqGSIb3DQEJBTEPFw0yNDAxMjMxMzE2NDFaMCsGCSqGSIb3DQEJ
# NDEeMBwwCwYJYIZIAWUDBAIBoQ0GCSqGSIb3DQEBCwUAMC8GCSqGSIb3DQEJBDEi
# BCDfGraj2ZesT4JGIE2RHLn76VGowlGnGfL4rfF65mcIMjCBpAYLKoZIhvcNAQkQ
...
# p6EZTQwcV7gpMA0GCSqGSIb3DQEBCwUABIIBgJyi8uqLqK6KUPMkVrcnEyZsxJu1
# kCvyUYb1F5pcB3hBcKJZ2W18IuyXrc2ONLk3trbHrR4npJQVYvpEx+8daaWjrAbH
# dymXlmac815y+oPuiGP88c5+fQRWvij0aVySXQeqcib2fS3700hs6rl5Dcz2vNvK
# CL9mAv+HD5/KZoHYTvtUxLuEc6MHNeyOSp3lU3HqezLytyh9yVdO6MurmGPuwhRb
# EOJro1vXCAntJ7qMLgGkQeXO3DW1U8FV6w2u4eQE/TZocubKPolpl+K2dji8YMXR
# pMvdtv43JmYXiv1Hl3RzswQrFIbU2PS0Hp2n0OrYGkqFBFSfkopOhTbvySbzUvSK
# 2ckKNn/e4ulZkJX2MIBo9dMAWK/XoRJ3Bbxs1xjRCXQcNm9JQGMqwh/M5/HpBJmn
# +wa3GeroGlkaM7oo5JKBIWLhFG5JQJAqaHLAMxubw8WjRmXYyKEO4Iw28UirovVk
# UochwanfO2x4L4e7EXkXiMcQAIwLqQWLtI8FwQ==
