...
'@
		...
		$LS = if ($ByteArray.Length -gt $LnkSize) { $ByteArray[($LnkSize - 1) .. $ByteArray.Length] } else { $null }
		...
		if (!!$ShotcutInfo.Extradata)
		{
			...
			if ($LinkFlags.contains('HasExpString'))
			{
				...
				$TargetUnicode = Get-SafePath -Path $ShotcutInfo.ExtraData.EnvironmentVariableDataBlock.TargetUnicode
						...
						if ($typed[$p].Type -eq 'VT_BSTR')
						{
							try
							{
								...
								if (($vl + 4) -le $typed[$p].Value.length)
								{
									...
									New-Variable -Name "TypedProperty$($p)" -Value @("$($typed[$p].Type): ", $vs)
								}
								else
								{
									$valuehex = [System.BitConverter]::ToString($typed[$p].Value) -replace '-', ''
									New-Variable -Name "TypedProperty$($p)" -Value @("$($typed[$p].Type): ", $valuehex)
								}
							}
							catch
							{
			...
			if ($ShotcutInfo.ExtraData.TrackerDataBlock)
			{
				...
				<#	# Need Samples for this
				# ExtraData.VistaAndAboveIDListDataBlock - (Not seen one yet)
				if ($ShotcutInfo.ExtraData.VistaAndAboveIDListDataBlock)
				{
					$linktargets | Add-Member -MemberType NoteProperty -Name "VistaAndAboveIDListDataBlock" -Value $ShotcutInfo.ExtraData.VistaAndAboveIDListDataBlock.IDList
				}#>
				}
			...
			if ($results.count -ge 1)
			{
				...
				foreach ($offset in $offsets)
				{
					...
					if (!!$LNKData)
					{
						...
						if (!!$LNKData.LinkSlackLength -and $LNKData.LinkSlack -match '[a-fA-F1-9]')
						{
						...
						if ($LNKData.Link_Flags.contains('HasLinkInfo'))
						{
							...
							# CommonNetworkRelativeLink
							if ($LNKData.'Link Info Flags'.Contains('CommonNetworkRelativeLinkFlags'))
							{
								...
								{
									$CommonNetworkflags = @($LNKData.'Common Network Relative Link Flags'.ToString().split(',', [System.StringSplitOptions]::RemoveEmptyEntries))
								...
								# ValidNetType
								if ($LNKData.'Common Network Relative Link Flags'.ToString().Contains('ValidNetType'))
								{
								...
								# ValidDevice
								if ($LNKData.'Common Network Relative Link Flags'.ToString().Contains('ValidDevice'))
								{
						...
						if ($LNKData.Link_Flags.contains('EnableTargetMetadata'))
						{
						...
						if (!!$LNKData.LinkSlackLength -and $LNKData.LinkSlack -match '[a-fA-F1-9]')
						{
							$null = $LNKNode.Nodes.Add("$('Link Slack')", "Link Slack: $($LNKData.LinkSlack)")
							$LNKNode.Nodes["$('Link Slack')"].ToolTipText = [System.Text.RegularExpressions.Regex]::Replace($LNKData.LinkSlack, '(.){100}', "$('$0')`n")
				...
				if (!!$treeview2)
				{
					...
					{
						$treeview2.Nodes[0].Expand()
						$treeview2.Nodes[0].Nodes[1].Expand()
					}
				}
			}
		}
				...
				if ($LNKData -ne $null)
				{
					...
					if ($LNKData.Link_Flags.Contains('HasLinkInfo'))
					{
						...
						if ($LNKData.'Link Info Flags'.Contains('CommonNetworkRelativeLink'))
						{
							...
							if (![System.String]::IsNullOrEmpty($LNKData.'CommonNetworkRelativeLinkFlags'))
							{
								$CommonNetworkflags = @($LNKData.'CommonNetworkRelativeLinkFlags'.ToString().split(',') )
								foreach ($cnflag in $CommonNetworkflags)
								{
									$null = $CommonNetworkRelativeLinkFlagsNode.Nodes.Add("$($cnflag)", "Relative Link Flag: $($cnflag.Replace(' ', ''))")
								}
					...
					if (!!$LNKData.LinkSlackLength -and $LNKData.LinkSlack -match '[a-fA-F1-9]')
					{
						$null = $streamNode.Nodes.Add("$('Link Slack')", "Link Slack: $($LNKData.LinkSlack)")
	...
	$treeview2_NodeMouseHover=[System.Windows.Forms.TreeNodeMouseHoverEventHandler]{
	#Event Argument: $_ = [System.Windows.Forms.TreeNodeMouseHoverEventArgs]
		<#if (!!$_.Node.Text)
		{
	...
	$Form_Cleanup_FormClosed=
	{
		...
		try
		{
			...
			$treeview2.remove_NodeMouseHover($treeview2_NodeMouseHover)
...
QmCCCw=='))
	...
	$treeview2.add_NodeMouseHover($treeview2_NodeMouseHover)
	#
	# contextmenustrip1
	#
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCCnfUX41Qr1kUUq
...
# AQQBgjcCAQQwLwYJKoZIhvcNAQkEMSIEIPujNFu5Cg7gqfAUbEo2+xH9WQjZ1vMB
# VLOAPeNe90ODMA0GCSqGSIb3DQEBAQUABIICAE4T9GLEHuBPzaz836ei6f6u2etX
# oRct0/vIX5Nzf2RRgv1+O30ydD9EA/PKko/0DfHUF8srjSy2HcETsaq9Y8sZTPu/
# 1Z2CJlq+oARJKuRgsAiFpHbAw40OVWAOPRCVp3Q7K0i+CIojkSnO7hKOXuLvIfWe
# XYxRMUyMZEgo96+fdqFWxuNautGmastCyFihAkPRY51MaFXxFDSnRiYU26PvVMtH
# lEyhybm50VODd8AyqzxTDgkayyAG+vv6JOflW1211ojakLh2FY2TrF6t+8UgO2hv
# XCGJJKMyJmS0NiVFSzJul9i41lrmfY7HXSSMCR/w1VlHHGbTnXzpE4H3jSr+797h
# 6Y1zoWnuEtN6VKe4CrK+rZ7NEegpkzw/tjVvwMLDHrzxXw3K7VDJz3gI71sU+gnx
# AdO0RVYcPi3IVqz5uYMF8kMawQdTqoj1uwajGW9x9h9ijK9SuBnuPNJZKFUQiWjF
# 2rZfhlmGJUWUFHHMbZv6eJhkEs9muxigwFRxYKWDTFlYWYh3BGaqod2QSWGHwwbz
# 5Ga4XqZ3TJT4HZmhPpXFAmPMnINY9YYuQrPUnO7l58FqUWKQAnM7Q1dA/S6t1AzG
# zp3D/ul1xRPFdU93WaKFEkHPibsN0K9ZcT8cBjUYmHxGNA7gl+/zcBML4mqHO0Ac
# 8GMWEZLaZFD0Kq2OoYIDbDCCA2gGCSqGSIb3DQEJBjGCA1kwggNVAgEBMG8wWzEL
...
# hvcNAQcBMBwGCSqGSIb3DQEJBTEPFw0yMzAxMjMyMjQxMTNaMCsGCSqGSIb3DQEJ
# NDEeMBwwCwYJYIZIAWUDBAIBoQ0GCSqGSIb3DQEBCwUAMC8GCSqGSIb3DQEJBDEi
# BCC1bkYw2DvLZBvjplue2+aR2KkBK6FPMVBwpqfKUoHhlDCBpAYLKoZIhvcNAQkQ
...
# p6EZTQwcV7gpMA0GCSqGSIb3DQEBCwUABIIBgKtCIlCJljtu42d16rg5fXr5daH3
# pammzbL2Sd6hV8HJ/5XhvQ/ZO1ODwXlXGGI1p3prt5J5uysbZcKfockhyGL6KHDc
# RbsDVvmbUESAJ/8RISvs2FIQIQEKtCkc4qT61X7iiblde4ruTr0Eh9FHZPCOzSHg
# w8KqTxRdVoAEopesMztuzHCCdPh/6lT2reWGjX2CYgM/99nMmZ+IbOQjiNFHDgJE
# 8ZfKjV3hpwnOGOkzf9eEi0A6JIkdZjaszy8EFOP615CJfa7mYEPjM+zm86kyiDnP
# Ssf+JJoPE5alrFfZG4ZWvsq46FlIZvWpbY8ABbe1gOpIpjZh3ZFJAzyQUVbQuX/f
# T6QwXK0eLgjI4Hionoh18IlmckIsX9CeFZXIMyxZlBtAKBoJ4NhkTo79BjZ7eZlk
# uREPUKWvpmOcchHhjh0EMnkznwzgrdOrkPxk9yF3k8eOFlHWDiQ8GXAuxNN6Hq8m
# CdECmKsCOS+zQz4zlrboWnvkt81ckqZh5FwVOA==
