...
function Show-MainForm_psf
{
	...
	function DosDateTime-FromHex
	{
		...
		try { Get-Date "$($Date) $($Time)" -Format s }
		catch { $null}
	}
			...
			if ($pc.PropertyStorage.Count -ge 1)
			{
				for ($t = 0; $t -lt $pc.PropertyStorage.Count; $t++)
				{
					...
					$formatID = if (!!$FormatIdentifiers[$sproperty.FormatID.Guid.ToUpper()]) { "$($FormatIdentifiers[$sproperty.FormatID.Guid.ToUpper()]) [$($sproperty.FormatID.Guid.ToUpper())]" }
								else { $sproperty.FormatID.Guid.ToUpper() }
					...
					if ($sproperty.PropertyStorage.Count -ge 1)
					{
						for ($p = 0; $p -lt $sproperty.PropertyStorage.Count; $p++)
						{
							...
							elseif (($typed.Type.ToString().Contains('VT_BLOB') -or $typed.Type.ToString().Contains('VT_STORAGE')) -and $null -ne $typed.Value)
							{
								$tpvalue = $null
								$tpraw = [System.BitConverter]::ToString($typed.Value) -replace '-', ''
							...
							$fmid = "$($sproperty.FormatID.Guid.ToUpper())\$($sproperty.PropertyStorage[$p].ID)"
...
'@
		...
		if ($LinkFlags.contains('HasLinkInfo'))
		{
			...
			if (!!$ShortcutInfo.LinkInfo.CommonPathSuffix)
			{
				...
				$linktargets | Add-Member -MemberType NoteProperty -Name "CommonPathSuffix Unicode" -Value $CommonPathSuffixUnicode
			}
	...
	$AppIDs = @{
		...
		"9B9CDC69C1C24E2B" = "Windows Notepad x64"
			...
			if ($results.count -ge 1)
			{
				...
				foreach ($offset in $offsets)
				{
					...
					if (!!$LNKData)
					{
						...
						# Extradata -VistaAndAboveIDListDataBlock
						if (!!$LNKData.Vista_DisplayName)
						{
							$null = $LNKNode.Nodes.Add("VistaDisplayName", "VistaAndAbove DisplayName: $($LNKData.Vista_DisplayName)")
						}
						if (!!$LNKData.Vista_Path)
						{
							$null = $LNKNode.Nodes.Add("VistaPath", "VistaAndAbove Path: $($LNKData.Vista_Path)")
						}
		...
		if ($count -ge 1)
		{
			...
			foreach ($streaminf in $streams)
			{
				...
				if ($LNKData -ne $null)
				{
					...
					if (!!$LNKData.Vista_DisplayName)
					{
						$null = $streamNode.Nodes.Add("VistaDisplayName", "VistaAndAbove DisplayName: $($LNKData.Vista_DisplayName)")
					}
					if (!!$LNKData.Vista_Path)
					{
						$null = $streamNode.Nodes.Add("VistaPath", "VistaAndAbove Path: $($LNKData.Vista_Path)")
					}
...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCA2ZjudnYO2flCX
# dyepfSGGNve2yxOtReUNYcBr/Onl2qCCKI0wggQyMIIDGqADAgECAgEBMA0GCSqG
...
# AQQBgjcCAQQwLwYJKoZIhvcNAQkEMSIEIAeMjfxrXl98+qxNQYI4cc7yVM94L/oT
# uRCtLveEl23iMA0GCSqGSIb3DQEBAQUABIICACWQxEaDzV7cgUkoMkaF25gztVo5
# 3VxF4c66/hMnT0CWjJcNVOM7cQay86HOog5C8H2BiRYg5D/daorjSzIypIFudD9B
# ukATy+apqr0fQamqrB/BsvyNkDiR+qQI2Plan/soEKdOP4HT6LDzPUHOZAAhWiud
# hZbyxJg8CLO3TZMotB3bUhy9Act/hk2r2sVDeHP3GenZEVuosfxSlfCHi2UFDpha
# 9lwLvNiGrO8a/0rzFhcdC3v+FvLk8+8+4docAtBUL7RfwyBrVs7prRPZY7382KTv
# PQcxL/8dx1qDvlSYvvZP3E3RP9S8BIDXdVyTKHBNaI3R/+AjBuZVuWKMcd7FcmqR
# YDCJ1vLKBEHSUR6b2KmzxlBhnTeSICH8z0tKNaZnPJ/AyIJ4rgMi7v8sqWsfzkFN
# IcrbaC1ypRxfjUyO0SWVecak6qHHvqKxh3v33jPm1Mob2kuaQvqBexd7Wa/O7m4i
# POQROG3GdOurpz80awgWKP6xP9OtKMtAPb4eaBngpz7sn/+me4eQ1Amkp7SFAu7U
# aR9sbcYE3qfhLiLJ3wqyWVxXYEjhyOnzV9iKrzLfcweMQnldcswiGE9La+wpsw6/
# TbaVDcEwk60UzI6sdGItYO+QEDntlzkVpya5Da3hr22bIgGs3oH1vaqUKU57gKCv
# MNoLgRrHWJzNzQeVoYIDbDCCA2gGCSqGSIb3DQEJBjGCA1kwggNVAgEBMG8wWzEL
...
# hvcNAQcBMBwGCSqGSIb3DQEJBTEPFw0yMzAyMTcwMTQ2MjRaMCsGCSqGSIb3DQEJ
# NDEeMBwwCwYJYIZIAWUDBAIBoQ0GCSqGSIb3DQEBCwUAMC8GCSqGSIb3DQEJBDEi
# BCC1Cq1bZ6ayAq7fbgSjSLXe9A2shJD5txG72VPUEmyHKDCBpAYLKoZIhvcNAQkQ
...
# p6EZTQwcV7gpMA0GCSqGSIb3DQEBCwUABIIBgEqr85gp32Fb5d3prM1y5okOmrmY
# Ju2gJLgHRcu2v1d8dHp+/mAZVteDMTECTwBQoy3WD42rrafM4tm/1xrXEOFMSdWW
# uCH6bZvmbIlMrQTPQjDu0AN/hahGrNOjiO9mbdd5iC2BPLRjvp16bTxJ54jODq8f
# CmnWFpN13qyJxKHpC/pakvGkgpXT6wNjFfJ0E4TdLk3Vs1DqVQayoX9dp7jROzYi
# 5KOFx9ktVViRAqaYug9V34YfwDKwvNKlwOqLnAJIXqJNoqaUrMUTQqgGOEoFxha/
# /dOHlIxXEwkARW6G+2CU/kuh6al4VY5CD0s+vmhaBmhjgI6f8AN4DW3CquPXFG3X
# VYkWatE7m79ulJdW7eiovcuMDtNQXUFYN6fZwsI69hVoNl6i2Lwql2qBV1tp29au
# vc3R+aNdlZl1ofZFC7aIUEUGTzAKf5lieU4sd2mZFEsGjo7cEmHEWS5e+38lj22v
# tAG3IwYuBqjkYdIeX01dGKyye9lxu9N3fHFuug==
