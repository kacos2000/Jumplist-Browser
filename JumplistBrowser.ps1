...
'@
	...
	$formatIDHashTable = [Ordered]@{
		...
		"EAEE7F1D-6A33-44D1-9441-5F46DEF23198\9"     = "Device BIOS Version"
		"92506491-FF95-4724-A05A-5B81885A7C92\1"	 = "WSD MetadataClean"
		"92506491-FF95-4724-A05A-5B81885A7C92\2"	 = "WSD ServiceInfo"
		...
		"92506491-FF95-4724-A05A-5B81885A7C92\4100"  = "WSD AppSeqInstanceID"
		"92506491-FF95-4724-A05A-5B81885A7C92\4101"  = "WSD AppSeqSessionID"
		"92506491-FF95-4724-A05A-5B81885A7C92\4102"  = "WSD AppSeqMessageNumber"
		...
		"656a3bb3-ecc0-43fd-8477-4ae0404a96cd\12292" = "Devices Device Category"
	...
	#	"1E3EE840-BC2B-476C-8237-2ACD1A839B22\31"    = "PersistedQuery properties kind"
	}
	...
	function Get-Ext_00 # Variant
	{
		...
		elseif (([System.BitConverter]::ToString($ByteArray[4 .. 7]) -replace '-', '') -in ('00EEEBBE', 'BBAF933B', '26017719', '02087619', '14038719')) # PropertyStore 4
		{
			$Signature = [System.BitConverter]::ToString($ByteArray[4 .. 7]) -replace '-', ''
			$ItemIdListProperties = [PSCustomObject]@{
				'ItemIDSize' = $ItemIDSize
	...
	function Get-Ext_C3
	{
		...
		try
		{
			...
			$String = ($TargetString -split '\x00')
			
			$Path = $String[0]
			$Desription = $String[1]
			if ($String.count -ge 3 -and $String[2].length -gt 1)
			{
				$Name = $String[2]
				$start = $TargetString.IndexOf($String[2])
				$idx = $start + $String[2].Length
			}
			else
			{
				...
				#	$sproperty = $pc.PropertyStorage[$t]
					...
					if ($pc.PropertyStorage[$t].PropertyStorage.Count -ge 1)
					{
						for ($p = 0; $p -lt $pc.PropertyStorage[$t].PropertyStorage.Count; $p++)
						{
							...
							elseif ($typed.Value -is [System.DateTime] -and $null -ne $typed.Value)
							{
								try
								{
							...
							try
							{
								...
								elseif ($pc.PropertyStorage[$t].PropertyStorage[$p].Name -in ('ViewItemsFolder', 'PubItemFolder.ShellItem'<#, 'AutoList'#>))
								{
									$tpbytes = $typed.Value
								}
								elseif ($pc.PropertyStorage[$t].PropertyStorage[$p].Name -in $TypedPropertiesWithUnicodeValues -or $pc.PropertyStorage[$t].PropertyStorage[$p].Name.StartsWith('Accounts')) # Unicode string
								{
									if ($typed.Value.length -gt 8)
									{
										try
...
'@
	...
	$CopyFullFilePath_Click = {
		if (!!$treeview1.SelectedNode -and !!$treeview1.SelectedNode.Tag -and $treeview1.SelectedNode.Tag.count -lt 3)
		{
			try
			{
...

...
Af8DQQGgDAADHwEsA20B/wNxAf8D0gH/ASsByQFiAf8DcQF8AwMBBBAAAxIB/wNvAf8DbwH/AyMB
...

...

...

...
+AHqAf8BnQGTAYQB/wMLAQ4DBAEFCAABCwEMAQsBDgb/AfsD/wH7A/8B+wH/Af4B/wH4Af8B+wH/
AfUB/wH4Af8B8gH/AfUB/wHvAf8B8QH/AesB/wHuAf8B6AX/AwIBAwgABAIDEwEXCP8BAAFnAQMB
...
'))
	#endregion
...

...
# KX7zUQIBAAIBAAIBAAIBAAIBADAxMA0GCWCGSAFlAwQCAQUABCAJRrVBCoapEpS6
# 3S5yV6q7O5BUEH40/BkJ/7nrSPnyiaCCKI0wggQyMIIDGqADAgECAgEBMA0GCSqG
...
# AQQBgjcCAQQwLwYJKoZIhvcNAQkEMSIEIHgK/xtIKhdm6R30yE9+gZ1u5nQ8qRmI
# 5miGKT9UX5yaMA0GCSqGSIb3DQEBAQUABIICAG2A6Xrja5joK72rsZTqY7Ebv28o
# PnCT7ee2dIaXfrdzAT6hzyFx2FTyhTdQY3tLgckS14s+1LOd0BOipLq3U62vhK+d
# XkJvmRI21LwAYSWvJQshha00qjybTE0MVwRU25AJvEx4EuUcSHi8ciqSo65oApiZ
# GCvFwxjNANg+BLnOKqqzAOJaRa2mMpR3W+K+zKCVz4sStRcuVzEZSIJymdV+nNXu
# DVFyjKNAGXcsa02C1uRayJ10M33I6bhXvdH0e/oise/NUQ/BLL1F2kKa22D99Spq
# 5gZxKNmb8gvKXvKii6rCgIeCUmyj4PBUPMMFQLEWzkOuKDHRX8iE8IlObqk4d55w
# jUzuSjlHFiqJ8NOrN0o4z8KCbAN/0Nelu6WEGeAGESSMgtZ6bNRBPpNaFQl/S2z0
# E+pZN5CbHQfA8p7QhuFHXkmEj16fSJ/huzp3zg6+Ppudc/6ggM8EdWYROOIwbYP4
# aEMvcgBH0UCg1xsr7SJiCQRI4iYaGZ8tKHBCLKxPCt+LWGUEDmR5zK791JOxGWK6
# iRNh5LOwPGc60cQPr8fP7aXG7lRREdbxNWvAeJ+W/z66YaNFhTyav99wr7gMwUdh
# v9JsY/G/lJ5r9CbI+C2ZBcuM8NzC+8nlqju6+NlYk7bBP5030QUq4mqSZ+lbHIOs
# FaBgZMMVdq73HvYloYIDbDCCA2gGCSqGSIb3DQEJBjGCA1kwggNVAgEBMG8wWzEL
...
# hvcNAQcBMBwGCSqGSIb3DQEJBTEPFw0yMzAyMjgyMjMzMjBaMCsGCSqGSIb3DQEJ
# NDEeMBwwCwYJYIZIAWUDBAIBoQ0GCSqGSIb3DQEBCwUAMC8GCSqGSIb3DQEJBDEi
# BCAP39Wxk4fJMpxxg+DCOB3+ibDm86wf1KJeXqYGhBzAMDCBpAYLKoZIhvcNAQkQ
...
# p6EZTQwcV7gpMA0GCSqGSIb3DQEBCwUABIIBgGzSiL/iku2yT0sp6o3FsC0ZT/45
# JbUE2YtdqKUawQPx/Ic7joWTctePGD3T1GbhTWBMVrZCR4TceJU8ZsxXbucmPXKY
# K7g0WiYoB3aLn2bOyabniPqIP3hUHpo666SBodQsJ9DqcgXwEoWGohDG1v3X+8rW
# HOo+dYL+FOPALiHGaEt2ku5r6pCTAmPBd1fvVc+q/z/QDb80qVWgfe3Z0Z4hrgYI
# RfllB3v8tHbjJppsGGFpsx4AdHu5ukqQvmPIbOt5e7V1REXALa+exCyKQMnHRxU9
# eqkR8VKm+wuHFg4lmzaU6Z3CIieuGTnWZX1z2TB4FNu19FIYKd+pCQoLHuRhEU0o
# jkrSIgJwetuwZmhhnlOpWANsLdtmACXoYXzCVVPuLh576frvG+pepkIJ8ySo6TSE
# SBKpuEqeUhJ5EOwoGoerFsM3G2EddSRORUaq4Ycvyxtw6S8YjueCzEyT/vSX3P64
# 2jyTBhnnSIr53Z3rIo/WhhTN+8RM9HXkhUj0YA==
